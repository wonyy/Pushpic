...
- (void)focusAtPoint:(CGPoint)point inFrame:(CGRect)frame
{
    if (_videoInput.device.isFocusPointOfInterestSupported && [_videoInput.device isFocusModeSupported:AVCaptureFocusModeAutoFocus]) {
        ...
        if ([_videoInput.device lockForConfiguration:&error]) {
            ...
            if ([_videoInput.device isFocusModeSupported: AVCaptureFocusModeAutoFocus]) {
                _videoInput.device.focusPointOfInterest = focusPoint;
                _videoInput.device.focusMode = AVCaptureFocusModeAutoFocus;
            }
         
            if ([_videoInput.device isExposureModeSupported: AVCaptureExposureModeAutoExpose]) {
                _videoInput.device.exposurePointOfInterest = focusPoint;
                _videoInput.device.exposureMode = AVCaptureExposureModeAutoExpose;
            } else if ([_videoInput.device isExposureModeSupported: AVCaptureExposureModeContinuousAutoExposure]) {
                _videoInput.device.exposurePointOfInterest = focusPoint;
                _videoInput.device.exposureMode = AVCaptureExposureModeContinuousAutoExposure;
            }
